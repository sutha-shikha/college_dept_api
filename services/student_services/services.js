...
Services.prototype.fetchAllUser=function(query,table,callback)
{
...
Services.prototype.delRow=function(query,table,callback)
{
	if(query!=null)
	this[table].where(query).destroy()
	.then(function(model) {
		model="SUCCESS";
		callback(model);
	})
...
Services.prototype.editData=function(query,data,table,callback)
{
	
	this[table].where(query).save(data,{patch:true}).then(function(model) {
		callback(model.toJSON());
	});
